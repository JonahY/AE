...
if __name__ == '__main__':
    ...
    dataMat = loadmat(os.path.join(r'F:\PAC\Pure Ni-tension test--0.01-2-AE Vallen&PAC-20211115\stream\thershold cutting', 'Pure Ni-tension test-0.01-2-STREAM20211116-010729-134_ch1.mat'))
    fs = dataMat['Sampling rate'][0][0]
    trigger_time = dataMat['Trigger time'][0][0]
    sig_initial = dataMat['Voltage'][0]
    t_initial = np.array([i / fs for i in range(len(sig_initial))])

    staLen, overlap, staWin = 5, 1, 'hamming'
    t_str, t_end = 0, 1e7
    t = np.around(t_initial[int(t_str // t_initial[1]):int(t_end // t_initial[1]) + 1] - t_initial[int(t_str // t_initial[1])],decimals=1)
    ...
    IZCRT, ITU, alpha, t_backNoise = 0.1, 675, 1.2, 1e4
