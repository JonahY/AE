...
class UNetMulti3(nn.Module):
    def __init__(self, in_ch=3, out_ch=3):
        ...
        self.linear = nn.Linear(23 * 23, 2).to('cuda:1')
        self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self, x):
        ...
        e5 = self.feature1(e4)
        # a5 = e5.to('cuda:1')
        a6 = torch.cat([torch.mean(e5.squeeze(1)[:math.ceil(e5.squeeze(1).size()[0] * 0.88)], dim=0, keepdim=True),
                        torch.mean(e5.squeeze(1)[math.ceil(e5.squeeze(1).size()[0] * 0.88):], dim=0, keepdim=True)], 0)
        # print(a6.flatten(start_dim=1, end_dim=2).size())
        a7 = self.linear(a6.flatten(start_dim=1, end_dim=2))
        # print(self.softmax(a7))
