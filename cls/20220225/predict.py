...
class UNetMulti3_per(nn.Module):
    def __init__(self, in_ch=3, out_ch=3):
        super(UNetMulti3_per, self).__init__()
    ...
    def predict(self, loader):
        ...
        with torch.no_grad():
            for i, (images, _) in enumerate(tbar):
                encode, _, decoded = self.model(images.to(self.device))
                # print(encode.shape)
                res.append(decoded)
...
if __name__ == "__main__":
    ...
    # dataset
    parser.add_argument('--load_path', type=str, default='./2022-03-01T16-19-21-classify-fold0_best_0.0001909.pth')
    parser.add_argument('--img_path', type=str, default='/home/Yuanbincheng/data/Ni-tension test-pure-1-0.01-AE-20201030/train dataset_wsst_norm')
    config = parser.parse_args()
    print(config)

    loader = transforms.Compose([transforms.Resize(184), transforms.ToTensor()])
    ...
    df.to_csv('./Ni_features_184_184__4_GAP_Linear_origin256_new.csv', index=None)

    for i in tqdm(range(len(dataset))):
        fig = plt.figure(figsize=[1.92*2, 1.92], num='1')
        ax1 = plt.subplot(121)
        ...
        plt.margins(0, 0)
        plt.show()
        plt.savefig(os.path.join('./res_184_184_4_GAP_Linear_origin256_new', '%i.jpg' % (i+1)), pad_inches=0)
